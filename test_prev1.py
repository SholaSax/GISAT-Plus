...
def generate_nmrs():
    ...
    try:
        if script_box.get() == 'Treatment Line List':
            ...
            path = open_file
            df.to_excel(
                f"{path}/CIHP_Full_Pharmacy_Complement_" + str(val_linelist_gen_date) + "_" + str(result_fac) + ".xlsx",
            ...
            path = open_file
            df.to_excel(
                f"{path}/CIHP_LIMS_EMR_Daily_Report_" + str(val_linelist_gen_date) + "_" + str(result_fac) + ".xlsx",
...
gplus = PhotoImage(file="images/gisat_plus.png")
...
linelist_gen_date = DateEntry(frame_1, selectmode='day', date_pattern='dd/mm/yyyy', width=20, font=("Century Gothic",
                                                                                                    18, "bold"))
...
def truncate_linelist_details():
    ...
    my_cursor.execute(f"SELECT COUNT(`Pepid_datim`) FROM {table}")
    num = my_cursor.fetchall()
...
radio_sel1 = welcome.App1()
radio_sel2 = radio_sel1.radio_var.get()
print(radio_sel2)
if radio_sel2 == 2:
    script_box == "Full Pharmacy Complement"
else:
    print("Oh no!!!")
