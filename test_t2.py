...
def run_main_app():
    ...
    def backup_nmrs():
        ...
        try:
            ...
            progressbar.start()
            l2 = Label(topwindow, text='Please wait.........', font=('Helvetica bold', 15), bg='#252525', fg='#ffffff')
            l2.pack()
            ...
            # close small window
            topwindow.after(5000, lambda: topwindow.destroy())
            # topwindow.destroy()
    ...
    def generate_nmrs():
        ...
        try:
            if script_box.get() == 'Treatment Line List':
                ...
                # Start moving the indeterminate progress bar.
                progressbar.start()
                l2 = Label(topwindow, text='Please wait.........', font=('Helvetica bold', 15), bg='#252525',
                           fg='#ffffff')
                l2.pack()
        ...
        if script_box.get() == 'Treatment Line List':
                                               ...
                                               "Total No. of file(s) imported: " + str(num) + "\n"
                                                   ...
                                                   start_time) + "\n"
                                                                 "End time: " + str(end_time) + "\n"
                                                                                                "Total time: " + str(
                                                   total_time) + "\n"
                                                                 "File(s) successfully imported. \nTo continue, click 'OK' and relaunch app.")
            ...
            for i in files:
                ...
                try:
                    ...
                    errordata = str(e)
                    info = (errordata[:11190] + '..') if len(errordata) > 90 else errordata
                    messagebox.showerror('Error!!!', info)
    ...
    linelist_gen_date = DateEntry(frame_1, selectmode='day', date_pattern='dd/mm/yyyy', width=20,
                                  font=("Century Gothic",
                                        18, "bold"))
    ...
    val_date = DateEntry(frame_1, selectmode='day', date_pattern='dd/mm/yyyy', width=20,
    ...
    def start_merging():
        ...
        if count_wrong_date_ip > 0 and ip_box.get() != '' and state_box.get() == '' and surge_box.get() == '' and lga_box.get() == '' and is_on:
        ...
        elif count_wrong_date_state > 0 and ip_box.get() != '' and state_box.get() != '' and surge_box.get() == '' and lga_box.get() == '' and is_on:
            messagebox.showerror("Date validation error!!!",
        ...
        elif count_wrong_date_surge > 0 and ip_box.get() != '' and state_box.get() != '' and surge_box.get() != '' and lga_box.get() == '' and is_on:
            messagebox.showerror("Date validation error!!!",
        ...
        elif count_wrong_date_lga > 0 and ip_box.get() != '' and state_box.get() != '' and surge_box.get() != '' and lga_box.get() != '' and is_on:
            messagebox.showerror("Date validation error!!!",
            ...
            if count_art_enrolled_date > 0 or count_check_art_start_date > 0 or count_check_pickup_after_discon > 0 \
                    ...
                    and (
                    ip_box.get() != '' and state_box.get() == '' and surge_box.get() == '' and lga_box.get() == '' and is_on):
                msg = messagebox.askyesnocancel("GISAT-Plus Warning!!!",
                                                ...
                                                f"2. Total number of client(s) without ART Start Date are {count_check_art_start_date}.""\n"
                                                f"3. Total number of client(s) who picked up drug after discontinuation are {count_check_pickup_after_discon}.""\n"
                                                f"4. Total number of client(s) who had drug pickup after DEATH are {count_check_pickup_after_death}.""\n"
                                                f"5. Total number of client(s) whose CTT Form are incomplete are {count_check_incomplete_discon}.""\n"
                                                f"6. Total number of client(s) who had drug pickup before ART Commencement are {count_check_art_lastpickup}.""\n"
                                                "\n"
                                                "Click Yes to continue and No to cancel.")
                if msg == True and ip_box.get() == '' and state_box.get() == '' and surge_box.get() == '' and lga_box.get() == '' and is_on:
                    messagebox.showerror("No selection made.")
                elif msg == True and ip_box.get() != '' and state_box.get() == '' and surge_box.get() == '' and lga_box.get() == '' and is_on:
                    df = pd.read_sql(mysql_script.select_all_merge0, mydb)
                elif msg == True and ip_box.get() != '' and state_box.get() != '' and surge_box.get() == '' and lga_box.get() == '' and is_on:
                    df = pd.read_sql(mysql_script.select_all_merge1, mydb, params=(ipget(), statebeg()))
                elif msg == True and ip_box.get() != '' and state_box.get() != '' and surge_box.get() != '' and lga_box.get() == '' and is_on:
                    df = pd.read_sql(mysql_script.select_all_merge2, mydb, params=(ipget(), statebeg(), surgeget()))
                elif msg == True and ip_box.get() != '' and state_box.get() != '' and surge_box.get() != '' and lga_box.get() != '' and is_on:
                    df = pd.read_sql(mysql_script.select_all_merge3, mydb,
    ...
    def truncate_linelist_details():
        my_cursor.execute("SELECT COUNT(DISTINCT(`IP`)) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of IP: ")
        result1 = f'{lab}{num[0]}'
        '''
        No_rec1 = ck.CTkLabel(app, justify=tkinter.LEFT, text=result1,
                                         font=("Century Gothic", 10))
        No_rec1.pack(pady=(1, 0), padx=(1, 220))
        '''

        my_cursor.execute("SELECT COUNT(DISTINCT(`State`)) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of State: ")
        result2 = f'{lab}{num[0]}'
        '''
        No_rec = ck.CTkLabel(app, justify=tkinter.LEFT, text=result2,
                                        font=("Century Gothic", 10))
        No_rec.pack(pady=(1, 0), padx=(1, 200))
        '''

        my_cursor.execute("SELECT COUNT(DISTINCT(`SurgeCommand`)) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of Surge Command: ")
        result3 = f'{lab}{num[0]}'
        '''
        No_rec = ck.CTkLabel(app, justify=tkinter.LEFT, text=result3,
                                        font=("Century Gothic", 10))
        No_rec.pack(pady=(1, 0), padx=(1, 139))
        '''

        my_cursor.execute("SELECT COUNT(DISTINCT(`lga`)) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of LGA: ")
        result4 = f'{lab}{num[0]}'
        '''
        No_rec = ck.CTkLabel(app, justify=tkinter.LEFT, text=result4,
                                        font=("Century Gothic", 10))
        No_rec.pack(pady=(1, 0), padx=(1, 208))
        '''

        my_cursor.execute("SELECT COUNT(DISTINCT(`FacilityName`)) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of Facilty: ")
        result5 = f'{lab}{num[0]}'
        '''
        No_rec = ck.CTkLabel(app, justify=tkinter.LEFT, text=result5,
                                        font=("Century Gothic", 10))
        No_rec.pack(pady=(1, 0), padx=(1, 196))
        '''

        my_cursor.execute("SELECT COUNT(`Pepid_datim`) FROM `linelist`")
        num = my_cursor.fetchall()
        lab = ("Total No. of Records: ")
        result6 = f'{lab}{num[0]}'
        '''
        No_rec = ck.CTkLabel(app, text=result6,
                                        font=("Century Gothic", 10), justify=tkinter.LEFT)
        No_rec.pack(pady=(1, 0), padx=(1, 190))
        '''
